...
const DeleteEvents = () => {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_PUBLIC_URL}/v1/b2c/events/get`);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_PUBLIC_URL}/v1/b2c/del-events/${id}`);
      setImages(images.filter(image => image.id !== id)); // Filter out the deleted image
          ...
          images.map((image) => (
            <div key={image.id} className="col-lg-1 col-md-4 col-sm-12 row mb-4 g-3">
              <div className="card">
                ...
                <h5 className="card-title">{image.eventName}</h5>
